...
public class ODotaAPI {
    ...
    public WinLose getWinLose(long id) throws JsonSyntaxException, Exception {
        return getWinLose(id, new MatchFilter());
    }
    ...
    public List<MatchPreview> getMatches(long id) throws JsonSyntaxException, Exception {
        return getMatches(id, new MatchFilter());
    }

    public List<MatchPreview> getMatches(long id, MatchFilter filter) throws JsonSyntaxException, Exception {
        Type listType = new TypeToken<List<MatchPreview>>() {
        }.getType();
        return gson.fromJson(readUrl(API_HOST + "players/" + id + "/matches/" + filter.getFilter()), listType);
    }

    public List<HeroDetail> getHeroes(long id) throws JsonSyntaxException, Exception {
        return getHeroes(id, new MatchFilter());
    }

    public List<HeroDetail> getHeroes(long id, MatchFilter filter) throws JsonSyntaxException, Exception {
        Type listType = new TypeToken<List<HeroDetail>>() {
        }.getType();
    ...
    public List<PlayerFriend> getPeers(long id) throws JsonSyntaxException, Exception {
        Type listType = new TypeToken<List<PlayerFriend>>() {
        }.getType();
